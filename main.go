...
var numberOfRunningDevices int
var programMutex sync.RWMutex
...
func main() {
	...
	for _, device := range devices {
		go insertData(beginning, db, device, end)
	}
	time.Sleep(10 * time.Second)
	for programIsRunning {
		fmt.Println("Running " + strconv.Itoa(numberOfRunningDevices) + " devices")
...
func insertData(beginning time.Time, db *gorm.DB, device database.Device, end time.Time) {
	programMutex.Lock()
	numberOfRunningDevices += 1
	programMutex.Unlock()
	timer := time.Now()
	...
	fmt.Println(device.Name + " took " + time.Since(timer).String())
	programMutex.Lock()
	numberOfRunningDevices -= 1
	programMutex.Unlock()
}
